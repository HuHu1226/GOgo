...
def init_sidebar():
    """
    初始化侧边栏界面，展示街道信息，并提供操作按钮。
    """
    asr_text = ""
    with st.sidebar:
        # 标题
        st.markdown("## 大咖伴游")
        st.markdown("[大咖伴游 https://github.com/dashan336/TripBuddyAI/")
        ...
        with st.container(height=400, border=True):
            ...
            st.subheader("街道特点", divider="grey")
        ...
        st.button("返回街道页", on_click=on_btn_click, kwargs={"info": "返回街道页"})
...
def main(meta_instruction):
    ...
    # 说明
    st.info(
        "本项目是基于人工智能的文字、语音、视频生成领域搭建的伴游大模型。用户被授予使用此工具创建文字、语音、视频的自由，但用户在使用过程中应该遵守当地法律，并负责任地使用。开发人员不对用户可能的不当使用承担任何责任。",
        icon="❗",
    )
